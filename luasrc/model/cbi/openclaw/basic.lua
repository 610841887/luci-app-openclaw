...
act.cfgvalue = function(self, section)
	...
	html[#html+1] = 'var logEl=document.getElementById("setup-log-content");'
	html[#html+1] = 'var statusEl=document.getElementById("setup-log-status");'
	html[#html+1] = 'if(r.log){'
	html[#html+1] = '  var out=[]; var lines=r.log.split("\\n");'
	html[#html+1] = '  for(var i=0;i<lines.length;i++){ var p=lines[i].split("\\r"); out.push(p[p.length-1]); }'
	html[#html+1] = '  logEl.textContent=out.join("\\n");'
	html[#html+1] = '}'
	...
	html[#html+1] = 'var dirSel=document.querySelector(\'[name="cbid.openclaw.main.install_dir"]\');'
	html[#html+1] = 'var dirVal=dirSel?dirSel.value:"";'
	html[#html+1] = '(new XHR()).get("' .. ctl_url .. '?action="+action+"&install_dir="+encodeURIComponent(dirVal),null,function(x){'
	...
	html[#html+1] = 'var dirSel=document.querySelector(\'[name="cbid.openclaw.main.install_dir"]\');'
	html[#html+1] = 'var dirVal=dirSel?dirSel.value:"";'
	html[#html+1] = '(new XHR()).get("' .. uninstall_url .. '?install_dir="+encodeURIComponent(dirVal),null,function(x){'
